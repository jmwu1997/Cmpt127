...
/* Return the darkest color that appears in the/4/t2.c array; i.e. the
   smallest value
*/
...
{  
  ...
  for(y=0;y<rows;y++){
    for(x=0;x<cols;x++){
      *(array+(x+y*cols))=*(newarray+(y*cols+cols-x-1));
    }
  }
  free(newarray); 
}
		    ...
		    unsigned int rows )
{
  ...
  for(y=0;y<rows;y++){
    for(x=0;x<cols;x++){
      *(array+(x+y*cols))=*(newarray+(x+cols*(rows-y-1)));
    }
  }
  free(newarray); 
}
		       ...
		       double scale_factor )
{
  ...
  for(y=0;y<rows;y++){
    for(x=0;x<cols;x++){
      temp=scale_factor*get_pixel(array,cols,rows,x,y);
      if(temp>255) 
      	set_pixel(array,cols,rows,x,y,255);
      else 
      	set_pixel(array,cols,rows,x,y,round(temp));
     }
  }
}
           ...
           unsigned int rows )
{
  if(cols%2!=0) cols--;
  if(rows%2!=0) rows--;
  unsigned int newcols=cols/2,newrows=rows/2,x,y=0;
  ...
  for(y=0;y<newrows;y++){
    for(x=0;x<newcols;x++){
    	value=(float)(get_pixel(array,cols,rows,x*2,y*2)+get_pixel(array,cols,rows,x*2+1,y*2)+get_pixel(array,cols,rows,x*2,y*2+1)+get_pixel(array,cols,rows,x*2+1,y*2+1))/4;
      	set_pixel(newarray,newcols,newrows,x,y,(int)round(value));
    }
  }
  return newarray;
}
